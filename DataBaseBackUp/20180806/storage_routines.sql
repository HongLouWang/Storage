...
--
-- Temporary view structure for view `v_stuff_active`
--

DROP TABLE IF EXISTS `v_stuff_active`;
/*!50001 DROP VIEW IF EXISTS `v_stuff_active`*/;
SET @saved_cs_client     = @@character_set_client;
SET character_set_client = utf8mb4;
/*!50001 CREATE VIEW `v_stuff_active` AS SELECT 
 1 AS `L_id`,
 1 AS `L_Content`,
 1 AS `U_id`,
 1 AS `ServerTime`,
...
--
-- Final view structure for view `v_stuff_active`
--
...
/*!50001 VIEW `v_stuff_active` (`L_id`,`L_Content`,`U_id`,`ServerTime`,`LocalDate`,`Username`,`Firstname`) AS select `log`.`id` AS `L_id`,`log`.`Content` AS `L_Content`,`log`.`Uid` AS `U_id`,`log`.`ServerTime` AS `ServerTime`,`log`.`LocalDate` AS `LocalDate`,`stuff`.`Username` AS `Username`,`stuff`.`Firstname` AS `Firstname` from (`log` join `stuff`) where (`log`.`Uid` = `stuff`.`id`) */;
...
-- Dump completed on 2018-08-06 19:28:54
